...
@Service
@Transactional
public class SLUserService extends AbstractEntityService<SLUser, String> implements UserDetailsService, SLUserReadOnlyService {

    private final SLUserRepository slUserRepository;
    private final SLUserReadOnlyService slUserReadOnlyService;

    public SLUserService(RestEntityRepository<SLUser, String> entityRepository, SLUserRepository slUserRepository,
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        Optional<SLUser> userOptional = slUserRepository.findOne(username);

        if (!userOptional.isPresent()) {
            userOptional = slUserRepository.findByEmail(username.toLowerCase());
        }
    ...
    public void setCurrentUser(String username){
        ...
        SLUser newCurrentUser = findById(username).orElseThrow(
                () -> new NotFoundException("User not found")
        );

        SecurityContextHolder.getContext().setAuthentication(
                new UsernamePasswordAuthenticationToken(newCurrentUser, null, newCurrentUser.getAuthorities())
        );
    }
